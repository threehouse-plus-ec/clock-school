...
function formatFreq(f) {
  return f < 10 ? f.toFixed(2) : f < 100 ? f.toFixed(1) : f.toFixed(0);
}
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
        ...
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 18, fontWeight: 600, color }}>{formatFreq(freq)} <span style={{ fontSize: 11, color: C.textDim }}>Hz</span></span>
      </div>
      <input
...
export default function BeatLab() {
  ...
  useEffect(() => {
    if (gainRef.current) gainRef.current.gain.setValueAtTime(volume, audioCtxRef.current.currentTime);
  }, [volume]);
  ...
  const animate = useCallback(() => {
    ...
    if (wc && an) {
      // Size canvas buffer to match CSS layout (1:1, no DPI scaling for performance)
      if (wc.width !== wc.offsetWidth || wc.height !== wc.offsetHeight) {
    ...
    if (sc && an) {
      if (sc.width !== sc.offsetWidth || sc.height !== sc.offsetHeight) {
  ...
  const start = useCallback(async () => {
    ...
    setPlaying(true);
    animRef.current = requestAnimationFrame(animate);
  }, [playing, mode, freq1, freq2, volume, animate]);
  ...
  const record = () => {
    ...
    setLog((prev) => [...prev, { time: timestamp(), beat: beatFreq, f1: freq1, f2: freq2, mode }]);
  };
    ...
    <div style={{ background: C.bg, color: C.text, minHeight: "100vh", fontFamily: "'Source Serif 4', Georgia, serif" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", minHeight: "calc(100vh - 120px)" }}>
        ...
        <div style={{ width: 280, flexShrink: 0, padding: 20, borderRight: `1px solid ${C.panelBorder}` }}>
          ...
          {mode === "mic" && (
            <div style={{ padding: "12px 0", fontSize: 13, color: C.textDim, lineHeight: 1.6 }}>
              <p style={{ marginBottom: 8 }}>Microphone mode analyses audio from your device's microphone. Feed in two tones from speakers, tuning forks, or external signal generators.</p>
              <p style={{ fontSize: 11, color: C.textDim }}>For best results, disable echo cancellation and noise suppression in your browser's audio settings.</p>
              {micError && <p style={{ color: C.red, marginTop: 8 }}>{micError}</p>}
            </div>
          )}
          ...
          <div style={{ marginTop: 24, padding: 16, background: C.surface, border: `1px solid ${C.panelBorder}`, borderRadius: 4, textAlign: "center" }}>
            ...
            {mode === "internal" && (
              <div style={{ fontSize: 11, color: C.textDim, marginTop: 4 }}>|f₂ − f₁| = {formatFreq(Math.abs(freq2 - freq1))} Hz (exact)</div>
            )}
          </div>

          {/* Record */}
          <button onClick={record} disabled={!playing} style={{ ...btnStyle(false), width: "100%", marginTop: 12, opacity: playing ? 1 : 0.4 }}>
            ● Record Measurement
          </button>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
          {/* Waveform */}
          <div style={{ flex: 1, minHeight: 180, padding: "12px 16px 4px", position: "relative" }}>
          ...
          {/* Spectrum */}
          <div style={{ flex: 1, minHeight: 180, padding: "4px 16px 12px", position: "relative" }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: C.textDim, marginBottom: 4 }}>
              Frequency Spectrum
